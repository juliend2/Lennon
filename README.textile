...
h3. On your local machine (for development)

These instructions are for Mac or Linux (not tested on Linux but should work)

# First, install shotgun (so you don't need to restart your rack each time you make a change) : @sudo gem install shotgun@
# Copy config/database.yml.tpl to config/database.yml
...
# Issue a @shotgun config.ru@ to start the development server
# Login to @http://localhost:9393/admin@ and start entering some fake content for development purpose!

h3. On your own server

(This is the prefered way for deploying a Lennon blog.)

# You'll first need to copy config/database.yml.tpl to config/database.yml and modify the configurations. (replace every occurrence of "development" with "production")
